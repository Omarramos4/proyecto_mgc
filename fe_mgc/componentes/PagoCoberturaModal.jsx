...
const PagoCoberturaModal = ({ show, onClose, cobertura: initialCobertura }) => {
  ...
  const { sueldoGeneral, isrRate, issRate } = useMemo(() => {
    ...
    // Debug: mostrar configuraciones
    console.log('CONFIGURACIONES:', configData?.configuraciones);
    // Buscar por nombre o por id si no existe el campo nombre
    let isrConfig, issConfig;
    if (configData?.configuraciones) {
      isrConfig = configData.configuraciones.find(c => (c.nombre || '').toUpperCase().includes('ISR'));
      issConfig = configData.configuraciones.find(c => (c.nombre || '').toUpperCase().includes('ISS'));
      // Si no existe el campo nombre, buscar por id o por algún otro campo
      if (!isrConfig && configData.configuraciones.length > 0) {
        isrConfig = configData.configuraciones.find(c => c.id === '1');
      }
      if (!issConfig && configData.configuraciones.length > 1) {
        issConfig = configData.configuraciones.find(c => c.id === '2');
      }
    }
    // Debug: mostrar valores encontrados
    console.log('ISR config:', isrConfig);
    console.log('ISS config:', issConfig);
    // Convertir porcentaje a decimal si el valor tiene %
    const parseRate = (valor) => {
      if (!valor) return 0;
    ...
    return {
      ...
      isrRate: isrConfig ? parseRate(isrConfig.valor) : 0,
      issRate: issConfig ? parseRate(issConfig.valor) : 0,
    };
  }, [cobertura, configData, configLoading, configError]);

  const { isrCalculado, issCalculado, totalDeduccion, pagoNeto } = useMemo(() => {
    // Debug: mostrar valores usados para el cálculo
    console.log('CÁLCULO:', { sueldoGeneral, isrRate, issRate, relojMarcador });
    const isr = sueldoGeneral * isrRate;
    const iss = sueldoGeneral * issRate;
    const deducciones = isr + iss + (Number(relojMarcador) || 0);
  ...
  return (
    <>
      ...
      {createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-md z-[999999] flex justify-center items-center">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-6 w-full max-w-md m-4 z-[1000000]">
            <div className="flex justify-between items-center border-b pb-3 border-gray-200 dark:border-gray-700">
              <h2 className="text-sm font-bold text-gray-900 dark:text-white">Registro de Honorarios | Cobertura: #{cobertura.id}</h2>
              ...
              <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    value={relojMarcador}
                    onChange={(e) => setRelojMarcador(Number(e.target.value))}
...
export default PagoCoberturaModal;
